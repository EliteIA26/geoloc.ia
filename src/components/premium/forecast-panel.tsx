...
export default function ForecastPanel({
  ...
  if (estado === "loading")
    return <div className="glass-panel p-3 text-sm text-muted-foreground">Cargando pronóstico…</div>;
  if (estado === "error" || !data)
    return <div className="glass-panel p-3 text-sm text-muted-foreground">Pronóstico no disponible ahora.</div>;

  return (
    <div className="space-y-3">
      <h3 className="text-sm text-muted-foreground">Pronóstico 7 días</h3>
      <div className="flex gap-1 overflow-x-auto">
        {data.dias.map((d) => (
          <div
            key={d.fecha}
            className="glass-panel flex min-w-12 flex-col items-center px-2 py-1.5 text-center"
          >
            <span className="text-[11px] capitalize text-muted-foreground">{diaCorto(d.fecha)}</span>
            <span className="text-xs text-[var(--foreground)]">{Math.round(d.tmax)}°</span>
            <span className="text-[11px] text-muted-foreground">{Math.round(d.tmin)}°</span>
      ...
      {data.ventana.length > 0 && (
        <p className="glass-panel p-3 text-sm text-muted-foreground">
          <span className="text-primary">Buenos días para regar/aplicar:</span>{" "}
