...
export default function InsightHero({
  ...
  return (
    <div className="glass-panel p-5">
      <div className="mb-2.5 text-xs text-muted-foreground">{eyebrow}</div>
      <p className="m-0 text-[16px] leading-relaxed text-[var(--foreground)]">{titulo}</p>
      {chips.length > 0 && (
      ...
      {accion && (
        <p className="mt-3.5 border-t border-[var(--border)] pt-3 text-sm text-muted-foreground">{accion}</p>
      )}
