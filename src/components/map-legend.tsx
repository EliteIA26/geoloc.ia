...
export default function MapLegend({ layer }: { layer: LayerKey }) {
  ...
  return (
    <div className="w-60 rounded-xl border border-black/5 bg-white/90 p-3 shadow-lg backdrop-blur-sm">
      ...
      <ul className="space-y-1.5">
        {rows.map((r) => (
          <li key={r.label} className="flex items-center gap-2">
            <span
              className="h-3.5 w-5 shrink-0 rounded-sm ring-1 ring-black/10"
